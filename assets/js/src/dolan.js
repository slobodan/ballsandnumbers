  ...
  function drawEvents() {
    ...
    const eventLabels = singleEvents.append('text')
      ...
      .on('mouseenter', function (d, i) {
        const currentLabelId = d3.select(this).attr('id');
        const currentLabelOffset = jQuery(`#${currentLabelId}`).position();
        const currentLabelWidth = jQuery(`#${currentLabelId}`).width();
        let yPlacement;
        let textAlign;
        if (events.length - i <= 10) {
          yPlacement = (currentLabelOffset.left - jQuery(document).scrollLeft() - 200)
           + currentLabelWidth;
          textAlign = 'right';
        } else {
        ...
        tooltip.html(d.date + ' ' + d.desc)
          .style('visibility', 'visible')
          .style('top', `${(currentLabelOffset.top - jQuery(document).scrollTop()) + 20}px`)
          .style('left', `${yPlacement}px`)
